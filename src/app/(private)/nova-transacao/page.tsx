...
const NewTransactionPage = () => {
  ...
  const isExpenseDetailsValid = useMemo(() => {
    return (
      Boolean(expenseFormData.label.trim()) && Boolean(expenseFormData.category)
    );
  }, [expenseFormData]);
  ...
  const handleFinalSubmit = () => {
    if (!userId) {
      alert(
        'Usuário não autenticado. Faça login para registrar uma transação.'
      );
      return;
    }
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setCurrentStep((previousStep) => (previousStep + 1) as TransactionStep);
  };
